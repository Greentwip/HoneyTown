...
namespace MonoGame.Extended.Collisions
{
    ...
    public class CollisionGrid
    {
        ...
        public CollisionGrid(int[] data, int columns, int rows, int cellWidth, int cellHeight, Vector2 gridPosition)
        {
        ...
        {
            var normalPosition = new Vector2(position.X, position.Y) - GridPosition;

            var column = (int) (normalPosition.X /CellWidth);
            var row = (int) (normalPosition.Y /CellHeight);
            ...
            var collisionRectangle = new RectangleF(new Vector2(0, 0), new Size2(CellWidth * Columns, CellHeight * Rows));

            if(!collisionRectangle.Contains(new Point2(normalPosition.X, normalPosition.Y)))
            {
                return new CollisionGridCell(this, column, row, 0);
            }
